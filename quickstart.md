...
ここでは、生研式装置（東京大学生産技術研究所〔後に複合技術研究所に所属〕の佐藤 剛司氏により開発された AC サーボモータ＋ダブルクラッチ式載荷装置）を用いて飽和供試体の三軸圧縮試験を行う際の、DigitShow Modbusの基本的な使い方を示します。
...
- [載荷装置と三軸セルを接続した後②：PreConsolidation](#載荷装置と三軸セルを接続した後②preconsolidation)
...
- [圧密③：Step Controlの設定](#圧密③step-controlの設定)
...
- （推奨）AD/DAボードと通信するCOMポートを`--port=COM10`や`-p=COM6`のように指定します。AD/DAボードのポート番号は、デバイスマネージャー等で確認しましょう。
...
1. 左上の“AD Input”メニューで“Calibration Value”をクリックします。
2. キャリブレーション値を設定します。
   - 手動で各チャンネルの値を入力する場合：a, b, cの係数に値を入力し、“Apply”を押します。
   - ファイルを読み込む場合：“Import”をクリックして、`.json`ファイルを読み込みます。現在の設定を保存する場合、“Export”で保存することができます。
...
1. 左上の“Specimen”メニューで“Config”をクリックします。
2. “Initial”列の“Diameter”行と“Height”行に、供試体の初期寸法と高さを入力し、“copy to present”ボタンを押します。
...
## 載荷装置と三軸セルを接続した後②：PreConsolidation

PreConsolidationは所定の q （軸差応力[kPa]）を保つ制御コマンドです。主に供試体の飽和過程で使用します。デフォルトの設定では、 q=0 なので、等方状態になります。

1. メイン画面左下の“Control ID”を 1 にして、“Apply”をクリックします。
...
1. 左上の“DA Output”メニューで“Voltage Output”をクリックします。
2. “CH 3: EP Cell Pressure”に適当な値を入力して“Output”を押します。EPの出力を現在のセル圧に合わせてから、セルに接続します。
...
2. 左上の“Control”メニューで“Step Control”をクリックします。
...
5. 設定の一例は[圧密③：Step Controlの設定](#圧密③step-controlの設定)節の表の通りです（初期拘束圧$`20\,\mathrm{kPa}`$から、$`100\,\mathrm{kPa}`$で等方圧密し、排水条件で軸ひずみ$`15\,\%`$まで単調載荷する場合）：
   ...
   - Step Controlダイアログでは、タブ区切りの設定ファイルの読み込みや書き出しも可能です。
...
![制御の設定の入力画面。](img/DSM_stepctrl.png)

*図: 制御の設定の入力画面。*

## 圧密④：データ保存と制御の開始

1. メイン画面左下の“Control ID”を 15 にして“Apply”をクリックします。
...
1. 左上の“Control”メニューで“Step Control”をクリックします。
...
2. 左上の“Specimen”メニューで“Config”をクリックし、“Export”ボタンを押して、供試体の情報をファイルに保存します。
