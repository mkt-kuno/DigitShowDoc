...
ここでそのままインストールしないこと。"個別のコンポーネント"より、追加のパッケージのインストールが必須です。Linux向け`C++`や$C\sharp$など似た表記が多いので注意すること。
...
HX711のint16_t（i16）物理値からmV/V、もしくは$\mu\varepsilon$への定格出力値への変換については、以下の式を使用してください。
