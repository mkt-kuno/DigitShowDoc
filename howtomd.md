...
- インライン: `` $`...`$ `` で囲みます（例: `` $`h_\mathrm{init}`$ ``）。
  GitHub のインライン数式は `$...$` と `` $`...`$ `` の 2 通りが定義されていますが、`$...$` は環境によって正しく表示されないため、本ドキュメントでは `` $`...`$ `` に統一します。
...
数値に付く単位（`` $`100\,\mathrm{kPa}`$ ``、`` $`38400\,\mathrm{bps}`$ ``、`` $`16\,\mathrm{bit}`$ `` など）もインライン数式として `` $`...`$ `` 形式で記述します（`siunitx` の `\SI{100}{\kPa}` に代わる）。
ただし、信号名や単独の記号など、平文で自然に読めるもの（`TX-`、`C#` など）は数式にせず平文で記述して構いません。
