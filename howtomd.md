...
数値に付く単位（`100 kPa`、`38400 bps`、`16 bit` など）は、数式にせず平文で記述します。
`$100\,\mathrm{kPa}$` のような数式表記は GitHub 上で正しく表示されないことがあるため使用しません（`siunitx` の `\SI{100}{\kPa}` の直接の置き換えではなく、平文にします）。
数式が本質的に必要な箇所（変数を含む式や数式ブロック）でのみ `$...$` / `$$...$$` を使用します。
