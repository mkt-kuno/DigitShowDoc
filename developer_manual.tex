...
完全表記が\texttt{--usb\_cdc\_direct=}、短縮表記が\texttt{-ucd=}です。
...
完全表記が\texttt{--float\_intput\_reg=}、短縮表記が\texttt{-fir=}です。
``True''もしくは``true''、``1''を指定することでInputRegisterの精度がfloat32\_tになります。

大まかな動作の違いを以下に示します。
\begin{table}[htbp]
    \centering
    \caption{各ボードの機能概要\label{tab:board_version}}
    \begin{tabular}{rlrrrr}
        \toprule
        BoardName & AD & DA & USB-Direct & FloatInput & Other \\ \midrule
...
完全表記が\texttt{--port=}、短縮表記が\texttt{-p=}です。

ModbusRTUボードとの通信に使用するCOMポートを指定するための引数です。
\texttt{--port=COM10}や\texttt{-p=COM6}のように指定します。
...
完全表記が\texttt{--baudrate=}、短縮表記が\texttt{-b=}です。
ModbusRTUボードとの通信速度を指定するための引数です。基本的に設定する必要はありません。
Trio・Quartet・YamaninではないオリジナルのModbusボードを使用する際に利用してください。

\subsection{フォント}
完全表記が\texttt{--font=}、短縮表記が\texttt{-f=}です。
フォントの指定は、\texttt{--font="MS Gothic"} \texttt{--font="Source Han Code JP"}のように指定します。
...
\subsection{動作モード}
完全表記が\texttt{--mode=}、短縮表記が\texttt{-m=}です。
...
完全表記が\texttt{--invert\_motor\_enable=}、短縮表記が\texttt{-ime=}です。

完全表記が\texttt{--invert\_motor\_direction=}、短縮表記が\texttt{-imd=}です。
...
\begin{lstlisting}[language=C++,caption={CDigitShowModbus.cpp より抜粋 }]
...
完全表記が\texttt{--listen=}、短縮表記が\texttt{-l=}です。
Webサーバー機能を有効にするための引数です。
有効化された場合、実行ファイルと同一ディレクトリにある\texttt{www}フォルダ内のファイルをルートとして提供します。
引数では公開範囲とポートを指定します。
例として、そのパソコンからのみアクセス可能で、通常のHTTPポート80で公開する場合は、\texttt{--listen="localhost:80"}と指定します。
他のパソコンからもポート80をアクセス可能にする場合は、\texttt{--listen="0.0.0.0:80"}と指定します。
...
ここでそのままインストールしないこと。``個別のコンポーネント''より、追加のパッケージのインストールが必須です。Linux向け\texttt{C++}や$C\sharp$など似た表記が多いので注意すること。
...
\texttt{git clone --recurse-submodules https://github.com/mkt-kuno/DigitShowModbus.git}

分割する場合や、更新があった場合などは

\texttt{git clone https://github.com/mkt-kuno/DigitShowModbus.git}
...
\texttt{src/DigitShowModbusDoc.cpp} 及び \texttt{src/DigitShowModbusDoc.h} を起点として、\texttt{src/Control\_xxxx.cpp}と\texttt{src/Control\_xxxx.h}を編集します。
そもそもの前提知識として、自分にあった\texttt{C++}の教材で（なんでもいいです）、ポインタとクラスまで学んでおくことを強くお勧めします。
...
多くの人が修正を加えたいのは、\texttt{CDigitShowModbusDoc::ControlMain}での処理になると思います。
以下にIISモーターモードでの処理の例を示します。

IDが1（CONTROL\_TYPE\_PRECONSOLIDATION）の場合は、\texttt{Control\_PreConsolidation}を呼び出します。これは見ての通り、``Control from File''からではない先行圧密用の関数です。

IDが15（CONTROL\_TYPE\_STEP）の場合は、\texttt{Control\_FileCtrl\_xxxxx}関数群を呼び出します。これがメイン機能の ``Control from File''で実行されるプログラム群です。

本当に動いているか不安な場合は``Release''ビルドでもログの出る\texttt{spdlog::info}をどんどん追加して、ログを出力してください。
...
\begin{lstlisting}[language=C++,caption={ Control\_Motor.cpp より抜粋 }]
...
\begin{itemize}
    ...
    \item 通信速度：$38400\,\mathrm{bps}$
    \item 入力-HX711：$1\,\mathrm{ch}$/ic, 計$8\,\mathrm{ch}$, $16\,\mathrm{bit}$精度, 128倍ゲイン,$10\,\mathrm{Hz}$動作
    \item 入力-ADS1115：$4\,\mathrm{ch}$/ic, 計$16\,\mathrm{ch}$, $16\,\mathrm{bit}$精度, $64\,\mathrm{Hz}$動作（オプションで$128\,\mathrm{Hz}$）
    \item 出力-GP8403：$2\,\mathrm{ch}$/ic, 計6/$8\,\mathrm{ch}$, $12\,\mathrm{bit}$精度, オンデマンド動作
\end{itemize}
...
\begin{itemize}
    ...
    \item 通信方式：USBシリアル
    \item 通信プロトコル：Modbus RTU
    \item 通信速度：$38400\,\mathrm{bps}$（USBダイレクトのため、何でも良い）
    \item 入力-HX711：$1\,\mathrm{ch}$/ic, 計$8\,\mathrm{ch}$, $16\,\mathrm{bit}$精度, 128倍ゲイン, $80\,\mathrm{Hz}$動作, 単純加算平均（8サンプル）
    \item 入力-ADS1115：$4\,\mathrm{ch}$/ic, 計$16\,\mathrm{ch}$, $16\,\mathrm{bit}$精度, $128\,\mathrm{Hz}$動作
    \item 出力-GP8403：$2\,\mathrm{ch}$/ic, 計$8\,\mathrm{ch}$, $12\,\mathrm{bit}$精度, オンデマンド動作
\end{itemize}
...
出力される値は本来であれば、符号付き$24\,\mathrm{bit}$精度ですが、ノイズ等により実質的な性能が$16\,\mathrm{bit}$程度のため、本プロジェクトでは符号付き$16\,\mathrm{bit}$精度（int16\_t/i16）として扱っています。

動作電圧が$2.048\,\mathrm{V}$の場合の物理値と、ゲイン済み電圧、ゲイン前電圧、の関係はを表すと以下の表のようになります。

\begin{table}[htbp]
    \centering
    \caption{HX711の物理値と換算式\label{tab:hx711_data}}
    \begin{tabular}{rrrrrr}
        \toprule
        物理値 & 物理値  & 差圧（128倍）$[\mathrm{mV}]$ & 差圧$[\mathrm{mV}]$ & 電圧$[\mathrm{mV}]$ & 出力（E=2）$[\mathrm{mV/V}]$ \\ \midrule
...
HX711のint16\_t（i16）物理値から$\mathrm{mV/V}$、もしくは$\mu\varepsilon$への定格出力値への変換については、以下の式を使用してください。
...
\begin{table}[htbp]
    \centering
    \caption{ADS1115の物理値と換算式\label{tab:ads1115_data}}
    \begin{tabular}{rrr}
        \toprule
        物理値 & 物理値  & 電圧 \\ \midrule
        32767  & 0x7FFF &  6.144 \\
...
具体的には、指定値がuint16\_tの範囲を取り、$\mathrm{mV}$オーダーで出力電圧を指定します。

GP8403のuint16\_t（u16）物理値の換算表、換算式は以下の表のようになります。

\begin{table}[htbp]
    \centering
    \caption{GP8403の物理値と換算式\label{tab:gp8403_data}}
    \begin{tabular}{rrrr}
        \toprule
        物理値 & 物理値 & 出力電圧$[\mathrm{mV}]$ & 出力電圧$[\mathrm{V}]$ \\ \midrule
...
多くのブラウザでは、\texttt{index.html} を自動的に読み込むため、\texttt{www/index.html} を作成しておくと、ブラウザでアクセスした際に自動的に表示されます。
...
基本となるWebUIとして、\texttt{https://github.com/mkt-kuno/DigitShowWebview/}にて、
...
\texttt{/v1/health/} に GET リクエストを送ると、サーバーの正常性を確認できます。
状態とタイムスタンプのみがJSON形式で返されます。

\subsection{計測データの取得API（v1）}
\texttt{/v1/} に GET リクエストを送ると、現在の最新計測データを取得できます。
\texttt{localhost:80} 運用時であれば \texttt{http://localhost/v1/} でアクセスできます。
計測データはJSON形式で返されます。
以下は、計測データの例です。

\begin{lstlisting}[caption={計測データJSON 一部抜粋}]
{
  "control": {
    "cycle_state": 0,
    "mode": 0,
...
\texttt{/v1/preview/} に GET リクエストを送ると、プレビュー用データ配列を取得できます。
\texttt{localhost:80} 運用時であれば \texttt{http://localhost/v1/preview/} でアクセスできます。
計測データはJSON形式で返されます。
例えば\texttt{time}と\texttt{phy\_00}を取得したい場合、
\texttt{http://localhost/v1/preview?time\&phy\_00} のように指定します。

以下は、プレビュー用データ配列の例です。
\begin{lstlisting}[caption={プレビュー用データ配列JSON 一部抜粋}]
{
    "raw_00": {
