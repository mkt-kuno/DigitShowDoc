...
\emph{Target}で、実行ファイルのパスの後に、半角スペースを開けてから、以下に示す各種の起動時変数の指定を行ってください。図の例では\texttt{-p=COM3}でCOMポートを指定しています。

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.9\hsize]{img/DSM_shortcut.png}
    \caption{ショートカットのプロパティを利用した起動時変数の設定例。}
\end{figure}
...
同一PCでたくさんチャートを見たいだけなら\texttt{--listen="localhost:80"}を、
実験室の他のパソコンからも見たいなら\texttt{--listen="0.0.0.0:80"}を設定してください。
\texttt{https://github.com/mkt-kuno/DigitShowWebview/}のRelease版をダウンロードして、
実行中の\texttt{DigitShowModbus.exe}と同じフォルダに置くと、簡単にリモートモニタリングが可能です。
複雑な表示がしたい人は自分でコーディングしてください。
フォルダ構成は以下のようになります。

\begin{lstlisting}[caption={Webサーバー機能有効時のフォルダ構成例}]
DigitShowModbus.exe
modbus-5.dll
www/
├── index.html
...
点問の軸圧縮量から局所軸ひずみを求めます。ベディングエラー（「外部変位計などによってキャップや載荷ピストンの軸変位から求めた軸ひずみ」に含まれる測定誤差）の無い正確な軸ひずみを求めることができます。計測できる最大ひずみは$1\,\%$程度ですが、微小なひずみを正確に計測することができます。供試体の直径方向に対称に1組設置し、2つのLDTで計測された値の平均値を用います。なお、LDTの設置は任意であり、使用しなくてもプログラムの動作に影響はありません。
...
\begin{figure}[htbp]
    ...
    \caption{メイン画面。\label{fig:dsm_main_add_inst}}
\end{figure}
...
\begin{enumerate}
    ...
    \item Physical Valueでは、Raw Valueをキャリブレーション値を用いて物理量($\mathrm{N}$や$\mathrm{mm}$など)に変換した値を表示します。
    \item Parametersでは、応力ひずみや、供試体の現在の寸法、レファレンス寸法などが表示されます。これらの値は、Physical Valueや供試体寸法の情報をもとに計算された値です。
    \item Voltage Outでは、\ref{sec:output_channel}節に示すアナログ電圧出力値の情報が表示されます。
    \item Plotでは、Raw ValueやPhysical Value、Parametersの値の変化をグラフで表示します。データ保存中は最大512点まで表示されます。保存していないときは、最新60秒のデータが表示されます。
    \item System Log \& Control Info では、システム全体のエラーや警告ログ、現状のControl情報や、次のステップの情報などを閲覧できます。
    \item Current Settings \& Basic Settingsでは、Control Typeや、Samplingレートの設定や、制御とサンプリングの開始・停止を行うことができます。なお、Control TypeやSamplingレートの設定は、Setボタンを押すことで反映されます。
\end{enumerate}
...
\begin{figure}[htbp]
    ...
    \caption{DA Output - Voltage Outputウィンドウ。\label{fig:dsm_VoltOut}}
\end{figure}
...
\begin{figure}[htbp]
    ...
    \caption{Specimen - Configウィンドウ。\label{fig:dsm_SpecimenConfig}}
\end{figure}
TDB...
...
\begin{figure}[htbp]
    ...
    \caption{Control - Pre-Consolidationウィンドウ。\label{fig:dsm_PreCons}}
\end{figure}
...
\begin{figure}[htbp]
    ...
    \caption{Control - Control from fileウィンドウ。\label{fig:dsm_CtrlFile}}
\end{figure}
\begin{enumerate}
...
\begin{figure}[htbp]
    ...
    \caption{Other - Versionウィンドウ。\label{fig:dsm_Version}}
\end{figure}
...
\begin{figure}[htbp]
    ...
    \caption{Other - Environmental Variablesウィンドウ。\label{fig:dsm_env}}
\end{figure}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.9\hsize]{img/precon_q_rpm.png}
    \caption{Pre-Consolidationの動作のイメージ\label{fig:precon-q-rpm}}
\end{figure}

\subsection{Control from file - 0: Stop}
...
以下のパラメータが設定できます。
\begin{itemize}
    \item Args[00]: Motor クラッチOFF設定。0は何もしない、1はモータークラッチOFF
    \item Args[01]: Motor 回転方向設定。0は何もしない、1はモーターUP
    \item Args[02]: Motor 速度設定。0は何もしない、1は回転速度をゼロ
    \item Args[03]: EP Cell Pressure設定。0は何もしない、1はEPの出力電圧を0[V]
    \item Args[04]: EP Axial Pressure設定。0は何もしない、1はEPの出力電圧を0[V]
\end{itemize}
...
\begin{itemize}
    ...
    \item Args[04]: 軸ひずみのリミッター値。単位は\%。Args[03]でリミッターを有効化した場合に、Args[04]で設定された値を超えるひずみが計測された場合に次のStep No.に進みます。
...
\begin{itemize}
    \item Args[00]: 圧縮(compression)か伸長(extension)か。0が圧縮、1が伸長です。
    \item Args[01]: モーターの回転速度。単位はRPM。RPMと実際の載荷速度の関係は試験機および減速機の設定などによって異なるため、予め確認しておく必要があります。
    \item Args[02]: 軸ひずみの下限値。単位は\%。
    \item Args[03]: 軸ひずみの上限値。単位は\%。
    \item Args[04]: 何サイクル載荷を行うか。所定のサイクル数の載荷を終えた場合、次のStep No.に進みます。
    \item Args[05]: 半径方向の有効応力。単位はkPa。正の値が入力されたときのみ、EPでセル圧を制御することで、円周方向の有効応力を設定された値に保ちます（0以下の値が入力された場合、EPの出力圧力は変化しません）。逆に、非排水三軸試験を行う場合には、必ず0を入力してください。
\end{itemize}
